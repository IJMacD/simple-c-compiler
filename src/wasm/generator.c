...
char* generate_operator (ast_node *node) {
  ...
  switch (node->string_val[0]) {
    case '+':
      append(output, &offset, "\t\tf64.add\n");
      break;
    case '-':
      append(output, &offset, "\t\tf64.sub\n");
      break;
    case '*':
      append(output, &offset, "\t\tf64.mul\n");
      break;
    case '/':
      append(output, &offset, "\t\tf64.div\n");
      break;
    default:
